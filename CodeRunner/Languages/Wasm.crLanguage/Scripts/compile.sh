...
if [[ "$PWD" -ef "$CR_UNSAVED_DIR" ]]; then
	...
	rm -rf "$CR_UNSAVED_DIR/Wasm/" &>/dev/null
	mkdir "$CR_UNSAVED_DIR/Wasm"
	cp "$CR_FILENAME" "$CR_UNSAVED_DIR/Wasm/$CR_FILENAME"
	cd "Wasm"
	CR_FILENAME="Wasm/$CR_FILENAME"
fi
...
wat2wasm $CR_FILENAME -o "$CR_SUGGESTED_OUTPUT_FILE"
status=$?
if [ $status -eq 0 ]; then
	if [ "$wasm_tmp_dir" = true ]; then
		mv -f "$CR_UNSAVED_DIR/Wasm/"* "$CR_UNSAVED_DIR/"
	fi
