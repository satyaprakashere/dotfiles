...
out="$(ghc -o "$CR_SUGGESTED_OUTPUT_FILE" -hidir="$CR_TMPDIR" -odir="$CR_TMPDIR" "$CR_FILENAME" "${@:1}")"

status=$?
if [ $status -eq 0 ]; then
	if [ ! -x "$CR_SUGGESTED_OUTPUT_FILE" ]; then
		echo $out
		echo -e "\nCodeRunner Warning: Your code can't be run because there is no Main module. Please run the file in your project containing your Main module."
		exit 1
	fi
