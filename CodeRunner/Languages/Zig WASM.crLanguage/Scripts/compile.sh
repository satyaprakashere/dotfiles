...
s=$?
if [ $s -ne 0 ]; then
	>& 2 echo "To run Zig code, you need to have Zig compiler installed"
	exit $s
fi

zig build-exe -target wasm32-wasi -O ReleaseSmall "$CR_FILENAME" -femit-bin="$CR_SUGGESTED_OUTPUT_FILE" "${@:1}"
status=$?
