...
scalac "$CR_FILENAME" -include-runtime -d "$CR_SUGGESTED_OUTPUT_FILE" "${@:1}"
...
if [ $status -eq 0 ]; then
	...
	echo -e "\nTo run Scala code, you need to have the Scala compiler installed. You can get it at https://scala-lang.org/"
fi
exit $status
