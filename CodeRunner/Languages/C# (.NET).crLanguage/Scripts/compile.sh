...
out="$(dotnet build "${@:1}" 2>&1)"
build_status=$?
>&2 echo "$out"
if [ $build_status -eq 0 ]; then
	exit 0
elif [ $build_status -eq 127 ]; then
	echo -e "\nTo run C# code, you need to have .NET installed. You can get it at https://dotnet.microsoft.com/"
else
	if [[ $out =~ (MSB1003[^\w]|not contain a project) ]]; then
		echo -e "\nCodeRunner Error: To run C# code, your code must be part of a .NET project. To create a project, open a terminal in the directory for your project and run the command \"dotnet new console\""
	fi
fi
